...
namespace ff {
	...
	template<typename T>
	void Attribute<T>::setVBO() noexcept {
		//生成vbo
		glGenBuffers(1, &vbo);
		glBindBuffer(GL_ARRAY_BUFFER, vbo);
		int a = mData.size() * sizeof(T);
		glBufferData(GL_ARRAY_BUFFER, mData.size() * sizeof(T), mData.data(), toGL(mBufferAllocType));
