...
vec3 MultiScatterBRDF(float NdotL, float NdotV)
{
  ...
  vec3 E_i = texture2D(uBRDFLut, vec2(NdotV, uRoughness)).xyz;//NdotV接近1的地方有亮斑
  ...
  vec3 fms = ( vec3(1.0) - E_o ) * (vec3(1.0) - E_i) / ( PI * (vec3(1.0) - E_avg) );//在NdotL或NdotV接近1的地方，有小暗斑
...
void main(void) {
  ...
  Lo += BRDF * radiance * NdotL;//定向光，不用积分，点光源也不用，因为只有一个方向有值
  vec3 color = Lo;
